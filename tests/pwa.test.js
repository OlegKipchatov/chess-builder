...
test('Весь граф runtime-модулей согласован с v35 и доступен офлайн',()=>{
 ...
 for(const file of readdirSync(new URL('../dist/',import.meta.url)).filter(name=>name.endsWith('.js')&&name!=='sw.js')){
  ...
  for(const match of read(file).matchAll(/from\s*['"]\.\/([^'"]+)['"]/g)){
   ...
   assert.ok(sw.includes(`'./${name}'`),name);if(!name.startsWith('vendor/'))assert.equal(query,'v=36',`${file}: ${name}`);
  }
 }
...
test('Переход v35 → v36 загружает ресурсы без HTTP-кэша и удаляет только старые кэши приложения',async()=>{
 ...
 vm.runInNewContext(read('sw.js'),{
  ...
  caches:{open:async name=>{assert.equal(name,'chess-vault-v36');return {addAll:async requests=>stored.push(...requests)};},keys:async()=>['chess-vault-v35','chess-vault-v36','another-app'],delete:async name=>removed.push(name)}
 });
 ...
 for(const url of ['./app.js?v=36','./ui/dialog.js?v=36','./ui/styles/dialog.css?v=36','./engine.js?v=36'])assert.ok(stored.some(request=>request.url===url),url);
 handlers.activate({waitUntil:promise=>{pending=promise;}});await pending;
 assert.deepEqual(removed,['chess-vault-v35']);assert.equal(claimed,true);
});
