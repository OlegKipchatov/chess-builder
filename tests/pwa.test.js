...
test('Весь граф runtime-модулей согласован с v42 и доступен офлайн',()=>{
 const sw=read('sw.js');
 for(const file of readdirSync(new URL('../dist/',import.meta.url)).filter(name=>name.endsWith('.js')&&name!=='sw.js')){
  ...
  for(const match of read(file).matchAll(/from\s*['"]\.\/([^'"]+)['"]/g)){
   ...
   assert.ok(sw.includes(`'./${name}'`),name);if(!name.startsWith('vendor/'))assert.equal(query,'v=42',`${file}: ${name}`);
  }
 }
...
test('Переход v36 → v42 загружает ресурсы без HTTP-кэша и удаляет только старые кэши приложения',async()=>{
 ...
 vm.runInNewContext(read('sw.js'),{
  ...
  caches:{open:async name=>{assert.equal(name,'chess-vault-v42');return {addAll:async requests=>stored.push(...requests)};},keys:async()=>['chess-vault-v36','chess-vault-v42','another-app'],delete:async name=>removed.push(name)}
 });
 ...
 for(const url of ['./app.js?v=42','./ui/dialog.js?v=42','./ui/styles/dialog.css?v=42','./engine.js?v=42'])assert.ok(stored.some(request=>request.url===url),url);
...
const workerHarness = ({online=true,networkStatus=200}={}) => {
 const handlers={},requests=[],removed=[],entries=new Map([['./index.html','<html>cached-37</html>'],['./app.js?v=42','app-37']]);
 let activated=false;
 ...
 vm.runInNewContext(read('sw.js'),{
  ...
  caches:{open:async name=>{assert.equal(name,'chess-vault-v42');return cache;},keys:async()=>['chess-vault-v34','chess-vault-v35','chess-vault-v42','other-app'],delete:async key=>removed.push(key)},
