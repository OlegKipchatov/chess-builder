...
test('Весь граф runtime-модулей согласован с v29 и доступен офлайн',()=>{
 const sw=read('sw.js');
 for(const file of readdirSync(new URL('../dist/',import.meta.url)).filter(name=>name.endsWith('.js')&&name!=='sw.js')){
  ...
  for(const match of read(file).matchAll(/from\s*['"]\.\/([^'"]+)['"]/g)){
   ...
   assert.ok(sw.includes(`'./${name}'`),name);if(!name.startsWith('vendor/'))assert.equal(query,'v=29',`${file}: ${name}`);
  }
 }
...
test('Переход v28 → v29 загружает ресурсы без HTTP-кэша и удаляет только старые кэши приложения',async()=>{
 ...
 vm.runInNewContext(read('sw.js'),{
  ...
  caches:{open:async name=>{assert.equal(name,'chess-vault-v29');return {addAll:async requests=>stored.push(...requests)};},keys:async()=>['chess-vault-v28','chess-vault-v29','another-app'],delete:async name=>removed.push(name)}
 });
 ...
 for(const url of ['./app.js?v=29','./ui/dialog.js?v=29','./ui/styles/dialog.css?v=29','./engine.js?v=29'])assert.ok(stored.some(request=>request.url===url),url);
 handlers.activate({waitUntil:promise=>{pending=promise;}});await pending;
 assert.deepEqual(removed,['chess-vault-v28']);assert.equal(claimed,true);
});
