...
const profile={id:'humanized19-v1',targetElo:446,effectiveElo:460.4859445538423,seed:219237029};
const setup=t=>{
...
test('Медленный поиск сначала получает stop, затем принимает последний легальный ответ',t=>{
 ...
 assert.ok(x.commands.find(c=>c.startsWith('go ')).includes(`movetime ${C.moveTimeMs}`));
...
test('Полный промежуточный MultiPV остаётся доступен модели после тайм-аута',t=>{
 const x=setup(t);x.worker.onmessage({data:'uciok\nreadyok'});
...
test('PGN пользователя с точным seed рассчитывается реальным SF19 с интерактивным лимитом', {timeout:30000},async()=>{
