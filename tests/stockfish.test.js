...
test('Новые партии фиксируют Cognitive v2, старые активные партии мигрируют на него',()=>{const state=initialState();state.game=createStartedGame(state,()=>0);const profile={...state.game.engineProfile};state.rating.value=1600;assert.deepEqual(migrateState(state).game.engineProfile,profile);delete state.game.engineProfile;assert.equal(migrateState(state).game.engineProfile.id,'cognitive-v2');});
