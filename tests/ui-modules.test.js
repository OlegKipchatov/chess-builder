...
test('Все вложенные JS/CSS-модули существуют и входят в атомарный офлайн-кэш',()=>{
 ...
 for(const file of files(rootPath).filter(path=>/\.(js|css)$/.test(path)&&!path.endsWith('/sw.js'))){
  ...
  for(const match of imports){const [target,version]=match[1].split('?');assert.ok(existsSync(resolve(dirname(file),target)),`${name}: ${target}`);if(!target.includes('/vendor/'))assert.equal(version,'v=29',`${name}: ${target}`);}
 }
});
