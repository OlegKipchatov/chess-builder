...
import {newGame} from './state.js?v=3';
...
export const createStartedGame = state => ({...newGame(state.settings.mode,state.settings.difficulty),started:true,equipped:structuredClone(state.equipped)});
export const updatePreferences = (state, game, settings) => {
  ...
  const difficulty = ['easy','normal','hard'].includes(settings.difficulty) ? settings.difficulty : 'normal';
