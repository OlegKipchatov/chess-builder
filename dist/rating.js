import {targetFor} from './cognitive-model.js?v=42';
...
export const opponentFor = targetFor;
...
export const validRatingSnapshot = snapshot => snapshot && Number.isSafeInteger(snapshot.before) && snapshot.before>=100 && snapshot.before<=2400 && Number.isSafeInteger(snapshot.opponent) && snapshot.opponent>=100 && snapshot.opponent<=1600 && [32,64].includes(snapshot.k);
