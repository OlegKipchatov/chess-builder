...
export const validEngineProfile = profile => profile?.id==='humanized19-v1' ? validPlayStyle(profile.profile)&&Number.isFinite(profile.targetElo)&&profile.targetElo>=C.minHumanElo&&profile.targetElo<=C.maxHumanElo&&Number.isFinite(profile.effectiveElo)&&profile.effectiveElo>=C.minHumanElo&&profile.effectiveElo<=C.maxHumanElo&&Number.isInteger(profile.seed)&&profile.seed>=0&&profile.seed<=C.seedMax : ['stockfish18-v1','stockfish19-v1'].includes(profile?.id)&&Number.isInteger(profile.skill)&&profile.skill>=0&&profile.skill<=12&&Number.isInteger(profile.nodes)&&profile.nodes>=1500&&profile.nodes<=20000&&profile.milliseconds===1500;
