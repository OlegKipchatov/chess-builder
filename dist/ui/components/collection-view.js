import {TYPES,PIECE_NAMES,STYLES,ITEMS,itemById,craftCost} from '../../catalog.js?v=27';
import {pieceSVG,itemPreview} from '../../pieces.js?v=27';
import {presetEquipment} from '../../collection-model.js?v=27';
import {button,sectionHeader,emptyState,escapeHTML} from '../primitives.js?v=27';
import {equipmentPreview,equipmentRow,savedSetRow,rarityLabel} from './equipment.js?v=27';
const collectionItemRow = (state,item) => {
  ...
  return `<article id="collection-item-${item.id}" tabindex="-1" class="compact-item ${equipped?'equipped':''}"><div class="compact-item-art">${itemPreview(item)}</div><div class="compact-item-copy">${rarityLabel(item.rarity)}<h3>${escapeHTML(style.name)}</h3><span class="item-ownership">${owned?'В коллекции':'Ещё не получен'}</span></div>${button({...action,label:equipped?'✓ Выбрано':owned?'Выбрать':`${cost} ✧ · создать`,className:equipped?'equipped-button':'',disabled:equipped||(!owned&&state.shards<cost)})}</article>`;
};
const presetCollectionRow = (state,style) => {
  ...
  return equipmentRow({name:style.name,meta:rarityLabel(style.rarity),preview:equipmentPreview(equipment,state.owned),equipped,actions:`<span>${total}/7 получено</span>`+button({label:equipped?'Выбрана':total===7?'Выбрать':'Не собрана','data-preset':style.id,disabled:total<7||equipped,className:equipped?'equipped-button':''})});
};
const equipmentStrip = (state,view,type) => {
  ...
  return `<article class="equipment-strip"><div class="strip-label"><p class="eyebrow">ВЫБРАННЫЙ НАБОР</p>${view==='saved'?'':button({label:'Сохранить набор',variant:'ghost','data-save-set':true})}</div><div class="equipment-slots">${slots}</div></article>`;
};
const savedSets = state => sectionHeader({title:`Мои наборы ${state.sets.length}/12`,endContent:button({label:'Сохранить текущий','data-save-set':true})})+(state.sets.length?`<div class="collection-list">${state.sets.map(savedSetRow).join('')}</div>`:emptyState('Сохраните выбранные фигурки и доску как свой набор.'));
const itemCatalog = (state,type,ownedOnly) => {
  ...
  return `<div class="catalog-heading"><p>${type==='board'?'Доска выбирается независимо от фигур.':`${PIECE_NAMES[type]} · скин применяется к обеим сторонам.`}</p>${button({label:ownedOnly?'✓ Только полученные':'Все предметы',className:'filter-toggle','data-owned-only':true,'aria-pressed':String(ownedOnly)})}</div><div class="collection-items-list">${items.length?items.map(item=>collectionItemRow(state,item)).join(''):emptyState('Полученных предметов этого типа пока нет.')}</div>`;
};
