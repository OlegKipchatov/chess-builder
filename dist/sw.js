...
const VERSIONED=ASSETS.filter(path=>path.endsWith('.js')||path.endsWith('.css')).map(path=>path+'?v=40');
self.addEventListener('install',event=>event.waitUntil(caches.open(CACHE).then(cache=>cache.addAll([...ASSETS,...VERSIONED].map(path=>new Request(path,{cache:'reload'})))).then(()=>self.skipWaiting())));
...
self.addEventListener('fetch',event=>{
  ...
  // Navigation must not pin an online user to an old cached HTML shell.
  // Other resources are served only from this release's cache, never another worker's.
