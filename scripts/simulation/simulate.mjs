...
export const simulate = ({position,fen,botElo,iterations=1000,seed=20260912,config,selector=selectCandidate}) => {
 ...
 for(let i=0;i<iterations;i++){
  const candidate=selector(position.candidates,botElo,position.context,rng,config),reference=position.reference.candidates.find(c=>c.move===candidate.move);
